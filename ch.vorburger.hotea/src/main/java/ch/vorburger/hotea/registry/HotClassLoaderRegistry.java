...
public interface HotClassLoaderRegistry {
    ...
    // TODO how to find WHICH classLoader to replace on reload.. implement some sensible equals() - in a new
    // HotClassLoaderProvider?
    ...
    void addHotClassLoaderBuilder(HotClassLoaderBuilder builder);
     ...
     * @return instance. For hot reloading to work, you CAN NOT "hang on" (i.e. store in a class member instance field) what
     *             this returns, but should ask for it every time
     *
     * @throws HotClassLoaderRegistryException if more than one implementation/subclass was available
     */
    <T> List<Class<? extends T>> getImplementationClasses(Class<T> apiClass) throws HotClassLoaderRegistryException;

    /**
     * Get a single implementation.
     * This is just a convenience method. If you want to initialize your instances yourself given a class (e.g. for DI
     * dependency injection), then don't use this variant, but the one that just returns Class.
     *
     * @param apiClass Java interface (or parent class) who's implementations/subclasses should be found
     * @return instance. For hot reloading to work, you CAN NOT "hang on" (i.e. store in a class member instance field) what
     *             this returns, but should ask for it every time
     ...
     * @throws HotClassLoaderRegistryException if more than one implementation/subclass was available
     */
    <T> List<T> getImplementations(Class<T> apiClass) throws HotClassLoaderRegistryException;
     ...
     * @throws HotClassLoaderRegistryException if more than one implementation/subclass was available
     */
    <T> T getImplementation(Class<T> apiClass) throws HotClassLoaderRegistryException;
}
